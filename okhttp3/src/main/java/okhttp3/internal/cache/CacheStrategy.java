...
public final class CacheStrategy {
    ...
    public static boolean isCacheable(Response response, Request request) {
        ...
        switch (response.code()) {
            //WHG 对于 200, 203, 204, 300, 301, 404, 405, 410, 414, 501, 308 只判断是不是存在
    ...
    public static class Factory {
            ...
            if (cacheResponse != null) {
                ...
                for (int i = 0, size = headers.size(); i < size; i++) {
                    ...
                    if ("Date".equalsIgnoreCase(fieldName)) {
                        ...
                        etag = value;
                        //WHG 服务器用缓存响应请求，该缓存从产生到现在经过多长时间(秒)
        ...
        public CacheStrategy get() {
            ...
            //WHG 如果可以使用缓存，那networkRequest必定为null；指定了只使用缓存但是networkRequest又不为null，冲突。那就gg(拦截器返回504)
        ...
        private CacheStrategy getCandidate() {
            ...
            //WHG okhttp会保存ssl握手信息  Handshake ,如果这次发起了https请求，但是缓存的响应中没有握手信息，发起网络请求

            //WHG 2、https请求，但是没有握手信息,进行网络请求
            //Drop the cached response if it's missing a required handshake.
            if (request.isHttps() && cacheResponse.handshake() == null) {
                return new CacheStrategy(request, null);
            }

            //WHG 3、主要是通过响应码以及头部缓存控制字段判断响应能不能缓存，不能缓存那就进行网络请求
            ...
            //WHG 4、如果 请求包含：CacheControl:no-cache 需要与服务器验证缓存有效性
            ...
            //WHG 5、如果缓存响应中存在 Cache-Control:immutable 响应内容将一直不会改变,可以使用缓存
            CacheControl responseCaching = cacheResponse.cacheControl();
            if (responseCaching.immutable()) {
                return new CacheStrategy(null, cacheResponse);
            }

            //WHG 6、根据 缓存响应的 控制缓存的响应头 判断是否允许使用缓存
            ...
            if (requestCaching.maxAgeSeconds() != -1) {
                //WHG 如果请求中指定了 max-age 表示指定了能拿的缓存有效时长，就需要综合响应有效缓存时长与请求能拿缓存的时长，获得最小的能够使用响应缓存的时长
            ...
            if (!responseCaching.noCache() && ageMillis + minFreshMillis < freshMillis + maxStaleMillis) {
                ...
                //WHG 如果已过期，但未超过 过期后继续使用时长，那还可以继续使用，只用添加相应的头部字段
                if (ageMillis + minFreshMillis >= freshMillis) {
                    builder.addHeader("Warning", "110 HttpURLConnection \"Response is stale\"");
                }
                //WHG 如果缓存已超过一天并且响应中没有设置过期时间也需要添加警告
            ...
            //WHG 如果设置了 If-None-Match/If-Modified-Since 服务器是可能返回304(无修改)的,使用缓存的响应体
