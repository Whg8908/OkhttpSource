...
public final class RetryAndFollowUpInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        /**
         * WHG  管理类，维护了 与服务器的连接、数据流与请求三者的关系。真正使用的拦截器为 Connect
         */
        ...
        while (true) {
            ...
            try {
                //WHG 请求出现了异常，那么releaseConnection依旧为true。
                response = realChain.proceed(request, streamAllocation, null, null);
                releaseConnection = false;
            } catch (RouteException e) {
                //WHG 路由异常，连接未成功，请求还没发出去
                //WHG 两个异常都是根据`recover` 方法判断是否能够进行重试，如果返回`true`，则表示允许重试。
                ...
                //WHG 请求发出去了，但是和服务器通信失败了。(socket流正在读写数据的时候断开连接)
            ...
            //WHG 如果进过重试/重定向才成功的，则在本次响应中记录上次响应的情况
            ...
            //WHG 处理3和4xx的一些状态码，如301 302重定向
            //WHG 重定向
            // 如果此方法返回空，那就表示不需要再重定向了，直接返回响应；但是如果返回非空，那就要重新请求返回的`Request`，
    ...
    private boolean recover(IOException e, StreamAllocation streamAllocation,
        ...
        //WHG 1、在配置OkhttpClient是设置了不允许重试（默认允许），则一旦发生请求失败就不再重试
        //The application layer has forbidden retries.
        if (!client.retryOnConnectionFailure()) return false;

        //WHG 2、由于requestSendStarted只在http2的io异常中为true，先不管http2
    ...
    /**
     *  WHG
     * @param e 返回false，表示不重试，返回true，查看isRecoverable（）下面的方法
     * @param requestSendStarted  http2才有可能是 true http1一定是false
     * @return
     */
