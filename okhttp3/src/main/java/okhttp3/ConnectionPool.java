...
public final class ConnectionPool {
    ...
    private final Runnable cleanupRunnable = new Runnable() {
        ...
        public void run() {
            while (true) {
                ...
                if (waitNanos > 0) {
                    //WHG 因为等待是纳秒级，wait方法可以接收纳秒级控制，但是要把毫秒与纳秒分开
    ...
    /**
     * WHG 最多保存 5个 处于空闲状态的连接，连接的默认保活时间为 5分钟
    ...
    RealConnection get(Address address, StreamAllocation streamAllocation, Route route) {
        ...
        for (RealConnection connection : connections) {
            //WHG 要拿到的连接与连接池中的连接  连接的配置(dns/代理/域名等等)一致 就可以复用
    ...
    /**
     * WHG 对http2而言，多路复用去重(所有同一地址的请求都应该共享同一个TCP连接) 先不管
    ...
    boolean connectionBecameIdle(RealConnection connection) {
        assert (Thread.holdsLock(this));
        //WHG 比如 服务器返回 Connection: close ，那就会把这个连接关掉 (noNewStreams 设置为true)
        if (connection.noNewStreams || maxIdleConnections == 0) {
            connections.remove(connection);
            return true;
        } else {
            //WHG 唤醒wait的清理任务 开始工作
    ...
    long cleanup(long now) {
        ...
        synchronized (this) {
            ...
            //WHG 超过了保活时间(5分钟) 或者池内数量超过了(5个) 马上移除，然后返回0，表示不等待，马上再次检查清理
            if (longestIdleDurationNs >= this.keepAliveDurationNs
                ...
                //WHG 池内存在闲置连接，就等待 保活时间(5分钟)-最长闲置时间 =还能闲置多久 再检查
                return keepAliveDurationNs - longestIdleDurationNs;
            } else if (inUseConnectionCount > 0) {
                // All connections are in use. It'll be at least the keep alive duration 'til we
                // run again.
                //WHG 有使用中的连接，就等 5分钟 再检查
                return keepAliveDurationNs;
            } else {
                // No connections, idle or in use.
                //WHG 都不满足，可能池内没任何连接,直接停止清理(put后会再次启动)
    ...
    private int pruneAndGetAllocationCount(RealConnection connection, long now) {
        //WHG 这个连接被使用就会创建一个弱引用放入集合，这个集合不为空就表示这个连接正在被使用
